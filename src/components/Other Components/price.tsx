...
const Div: React.FC<DivProps> = ({ data, ind }) => {
  return (
    <div
      className={`h-[300px] w-[280px] max-900:h-[275px] max-900:w-[265px] max-800:h-[240px] max-800:w-[220px] max-700:h-[185px] max-700:w-[185px] ${ind === 1 ? 'bg-[#000] text-[#fff] z-10' : 'bg-[#f8f8f8] text-[#27363d] shadow-custom'
        } rounded-[40px] text-left p-[30px] relative ${ind === 0 ? 'rotate-[-8deg] top-[10px]' : ind === 2 ? 'rotate-[8deg] top-[10px]' : ''
        }`}
    >
      <p className="text-[33px] max-900:text-[30px] max-800:text-[26px] max-700:text-[21px]">{data.category}</p>
      <p className="text-[19px] max-900:text-[17px] max-800:text-[15px] max-700:text-[9px]">
        <span className="relative bottom-[11px] right-[5px] text-[20px] max-900:text-[18px] max-800:bottom-[9px] max-700:text-[15px] max-700:bottom-[6px]">₹</span>
        <span className={`text-[40px] max-900:text-[35px] max-800:text-[31px] max-700:text-[25px] `}>{data.price}</span>/
        {data.period}
      </p>
      <p className="absolute bottom-[30px] text-[16px] max-900:text-[15px] max-800:text-[15px] max-700:text-[9px] left-5">{data.Tagline}</p>
    </div>
  );
};
  ...
  return (
    <>
      <div className="text-center w-screen flex flex-col items-center py-[50px]  bg-white ">
        ...
        <p className="w-[600px] text-[#787878] text-[17.5px] mt-[10px] max-900:text-[17px] max-800:text-[16px] max-700:text-[12px]">
        ...
        <div className="flex mt-[40px] max-:flex-col ">
          {period.map((val, ind) => (
            <Div key={ind} data={val} ind={ind} />
          ))}
        </div>
        <div className="mt-14">
          <h1 className="text-[#27363d] text-[25px] font-semibold max-900:text-[23px] max-700:text-[22px]">All Plans Include</h1>
          <div className="flex gap-[100px] max-800:flex-col max-900:gap-[70px] max-800:gap-[12px] mt-[23px] text-[#000]">
            <ul>
              <li className="w-[370px] max-800:w-[600px] max-700:w-max max-900:w-[350px]  text-left text-[17px] max-900:text-[16px] max-800:text-[18px] max-700:text-[15px] relative before:absolute before:content-[url('/src/assets/circle-check.png')] before:left-[-40px] before:top-[1.5px]">
                Expert bookkeeping, automated reports, and tax assistance.
              </li>
              <li className="w-[370px] max-800:w-[600px] max-700:w-max max-900:w-[350px]  max-900:text-[16px] text-left text-[17px] max-800:text-[18px] max-700:text-[15px] mt-3 relative before:absolute before:content-[url('/src/assets/circle-check.png')] before:left-[-40px] before:top-[1.5px]">
            ...
            <ul>
              ...
              <li className="w-[370px] max-800:w-[600px] max-700:w-max max-900:w-[350px] text-left text-[17px] max-900:text-[16px] max-800:text-[18px] max-700:text-[15px] mt-3 relative before:absolute before:content-[url('/src/assets/circle-check.png')] before:left-[-40px] before:top-[1.5px]">
